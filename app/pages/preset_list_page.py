...
class PresetListPage(QWidget):
    ...
    def init_ui(self):
        ...
        # Date filter and search filter removed per user request
    ...
    # Filtering methods removed per user request
    ...
    def _on_wo_selected(self, wo_data):
        ...
        # self.profiles.append(new_profile) # REMOVED: Do not save yet
        # self.save_profiles()              # REMOVED: Do not save yet
    ...
    def fetch_sku_data(self):
        if not is_connected():
            msg = QMessageBox(self)
            msg.setWindowTitle("Koneksi Database")
            msg.setText("Tidak terhubung ke Database.\nGagal melakukan Sinkronisasi SKU.")
            msg.setStandardButtons(QMessageBox.Ok)
            msg.setIcon(QMessageBox.Warning)
            msg.setStyleSheet("QMessageBox { background-color: white; } QMessageBox QLabel { color: #333; font-size: 14px; }")
            msg.exec()
    ...
    def go_back(self):
        ...
        msg.setText("Apakah Anda yakin ingin kembali?")
        msg.setStandardButtons(QMessageBox.Yes | QMessageBox.No)
        msg.setStyleSheet("""
            QMessageBox { background-color: white; }
            QMessageBox QLabel { color: #333; font-size: 15px; padding: 12px; }
